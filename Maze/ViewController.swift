...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for y in 0 ..< maze.count{
            for x in 0 ..< maze[y].count{
                switch maze[y][x]{
                    case 1:
                        let wallView = createView(x: x, y: y, width: cellWidth, height: cellHeight,
                            offsetX: cellOffsetX, offsetY: cellOffsetY)
        ...
        playerView = UIView(frame: CGRectMake(0, 8, screenSize.width/50, screenSize.height/50))
    ...
    func startAccelerometer(){
        ...
        //加速度を取得する
        let handler: CMAccelerometerHandler = {(accelerometerData:CMAccelerometerData?, eroor: NSError?) -> Void in
            
            self.speedX += accelerometerData!.acceleration.x
            self.speedY += accelerometerData!.acceleration.x
